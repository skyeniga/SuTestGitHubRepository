namespace Application_Import_Logic {
    ...
    [Microsoft.XLANGs.BaseTypes.SchemaReference(@"Salesforce_Application_Import_Definition_XSD.CDataServiceContact+table", typeof(global::Salesforce_Application_Import_Definition_XSD.CDataServiceContact.table))]
    ...
    [Microsoft.XLANGs.BaseTypes.SchemaReference(@"Application_Import_Definition.HomeDisasterLoanApplicationCanonical", typeof(global::Application_Import_Definition.HomeDisasterLoanApplicationCanonical))]
    public sealed class MakeCanonicalWithApplicantSalesForceIDs : global::Microsoft.XLANGs.BaseTypes.TransformBase {
        
        private const string _strMap = @"<?xml version=""1.0"" encoding=""UTF-16""?>
<xsl:stylesheet xmlns:xsl=""http://www.w3.org/1999/XSL/Transform"" xmlns:msxsl=""urn:schemas-microsoft-com:xslt""
								xmlns:var=""http://schemas.microsoft.com/BizTalk/2003/var"" exclude-result-prefixes=""msxsl var s0 s1 userCSharp"" version=""1.0""
								xmlns:s1=""http://schemas.microsoft.com/BizTalk/2003/aggschema"" xmlns:s0=""http://sba-oda/DLAP/Contact""
								xmlns:ns0=""http://SBA.ODA.Gov/CanonicalApplication"" xmlns:userCSharp=""http://schemas.microsoft.com/BizTalk/2003/userCSharp"">
  <xsl:output omit-xml-declaration=""yes"" method=""xml"" version=""1.0"" />
  <xsl:template match=""/"">
    <xsl:apply-templates select=""/s1:Root"" />
    <xsl:variable name=""SeqCount"" select=""userCSharp:Iniatalize()""/>
  </xsl:template>
  <xsl:template match=""/s1:Root"">
    <ns0:Application>
      ...
      <ApplicantColl>
        <xsl:for-each select=""InputMessagePart_1/ns0:Application/ApplicantColl"">
          <xsl:for-each select=""Applicants"">
            ...
            <xsl:variable name=""var:v2"" select=""../Applicants[number($var:v1)]/APL_ORG_TYPE_CD/text()"" />
            <Applicants>
              <xsl:call-template name=""PopulateSalesForceID"">
                <xsl:with-param name=""SalesForceResponseRow"" select=""string($var:v1)"" />
                <xsl:with-param name=""APPType"" select=""string(../../APP_PRD_PRODUCT/text())"" />
                <xsl:with-param name=""ORGID"" select=""string($var:v2)"" />
              </xsl:call-template>
              <AppId>
              ...
              <APL_SalesForceAccountID>
                <xsl:value-of select=""APL_SalesForceAccountID/text()"" />
              </APL_SalesForceAccountID>
            </Applicants>
          </xsl:for-each>
        </xsl:for-each>
      ...
      <PaperApplication>
        <xsl:value-of select=""InputMessagePart_1/ns0:Application/PaperApplication/text()"" />
      </PaperApplication>
...
public int NextSeq()
{
 ...
 return Num;
  ...
  <xsl:template name=""PopulateSalesForceID"">
    ...
    <xsl:choose>
      <xsl:when test=""$ORGID ='IND' and $APPType ='Business'"">
        <xsl:variable name=""INDCount"" select=""userCSharp:NextSeq()""/>
        <xsl:element name=""SalesForceContactID"">
          <xsl:value-of select=""/*[local-name()='Root' and namespace-uri()='http://schemas.microsoft.com/BizTalk/2003/aggschema']/*[local-name()='InputMessagePart_0' and namespace-uri()='']/*[local-name()='table' and namespace-uri()='http://sba-oda/DLAP/Contact']/*[local-name()='row' and namespace-uri()='http://sba-oda/DLAP/Contact'][number($INDCount)]/*[local-name()='Id' and namespace-uri()='http://sba-oda/DLAP/Contact']/text()"" />
        </xsl:element>
      </xsl:when>
      <xsl:when test=""$APPType ='Home'"">
        <xsl:element name=""SalesForceContactID"">
          <xsl:value-of select=""/*[local-name()='Root' and namespace-uri()='http://schemas.microsoft.com/BizTalk/2003/aggschema']/*[local-name()='InputMessagePart_0' and namespace-uri()='']/*[local-name()='table' and namespace-uri()='http://sba-oda/DLAP/Contact']/*[local-name()='row' and namespace-uri()='http://sba-oda/DLAP/Contact'][number($SalesForceResponseRow)]/*[local-name()='Id' and namespace-uri()='http://sba-oda/DLAP/Contact']/text()"" />
        </xsl:element>
      </xsl:when>
      <xsl:otherwise>
        <xsl:element name=""SalesForceContactID"">
        ...
        private const string _strSrcSchemasList0 = @"Salesforce_Application_Import_Definition_XSD.CDataServiceContact+table";
        
        private const global::Salesforce_Application_Import_Definition_XSD.CDataServiceContact.table _srcSchemaTypeReference0 = null;
        ...
        public override string[] SourceSchemas {
            get {
                ...
                _SrcSchemas[0] = @"Salesforce_Application_Import_Definition_XSD.CDataServiceContact+table";
